let FinancialController = {};

/*-------------------------------------------------------------------------*
 *              GeneralManager functions && settings                       *
 *-------------------------------------------------------------------------*/

FinancialController.addCommas = ( nStr ) => {
    nStr += '';
    x = nStr.split('.');
    x1 = x[0];
    x2 = x.length > 1 ? '.' + x[1] : '';
...
FinancialController.defaultPieSettings = function(){
    ...
    cd.tooltipFormat = function f(){
        let series = this.getSeriesLabel(),
            ...
            value = FinancialController.addCommas(this.getValue().toFixed(2));
...
FinancialController.defaultBarSettings = function(){
    let cd = this.chartDefinition;
    // general settings
